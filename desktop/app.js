(function ($) {
 ...
 ` }
    ...
    const windowsMap = {};
    function normalStateSVG() { return `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"> <rect x="7.5" y="0.5" width="16" height="16" rx="1.4" fill="none" stroke="currentColor" stroke-width="2.5"></rect> <rect x="1.7" y="4.5" width="16" height="16" rx="1.5" fill="#ffffff" stroke="currentColor" stroke-width="2.5"></rect><rect x="2.5" y="5" width="15" height="15" rx="1.5" fill="rgb(1 121 214)" stroke="currentColor" stroke-width="1"></rect>`; }
    ...
    function minIconSVG() { return `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"> <path d="M5 12.1h23" stroke="currentColor" stroke-width="3.3" stroke-linecap="round"></path> </svg>`; }
    function closeIconSVG() { return `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"> <path d="M4.17157288 4.17157288 L19.82842712 19.82842712 M19.82842712 4.17157288 L4.17157288 19.82842712" stroke="currentColor" stroke-width="3.3" stroke-linecap="round" stroke-linejoin="round"/> </svg>`; }
...
// 修改 minimizeWindow：立即标记为 minimized，动画仍然播放
function minimizeWindow(id) {
  ...
  requestAnimationFrame(() => {
    $ghost.css({ transform: `translate(${translateX}px, ${translateY}px) scale(${scaleX}, ${scaleY})`, opacity: 0.95 });
  });
  setTimeout(() => {
    $ghost.remove();
    // 保持 meta.minimized 已为 true；隐藏 DOM
  ...
  `;
    ...
    function restoreFromTask(id) {
        ...
        if (willMaximize && meta.prevRect) {
            meta.$el.addClass('maxed-over').css({ position: 'fixed', left: 0, top: 0, width: '100vw', height: 'calc(100vh - 32px)', zIndex: 9999, visibility: 'visible' }).show();
            meta.maximized = true;
        } else {
    ...
    function applyMaxOver(id, forceApply) {
        ...
        meta.maximized = true;
        meta.$el.addClass('maxed-over').css({ position: 'fixed', left: 0, top: 0, width: '100vw', height: 'calc(100vh - 32px)', zIndex: 9999, visibility: 'visible' });
    ...
    // 批量最小化：使用 id 快照并一次性持久化（避免遍历时状态变化影响判断）
    function minimizeAllWindows(){Object.keys(windowsMap).forEach(i=>{const n=windowsMap[i];n&&!n.minimized&&minimizeWindow(i)}),setTimeout(persistWindows,420)}
    // 批量恢复：使用 id 快照并直接调用 restoreFromTask（不依赖延迟标记）
    function restoreAllWindows(){Object.keys(windowsMap).forEach(o=>{const s=windowsMap[o];s&&(s.minimized?restoreFromTask(o):bringToFront(o))}),setTimeout(persistWindows,120)}
